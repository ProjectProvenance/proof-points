...
describe('IPFSProvider', () => {
  ...
  it('should return the digest when adding the doc', () => storageProvider.add(docInBytes).then((res) => {
    expect(res.digest).to.equal(digest);
  }));
  it('should return the doc when getting the digest', () => storageProvider.get(digest).then((res) => {
